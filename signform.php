        ...
        <div id='header_group' style="display:block; text-align: center;">
            ...
            <?php
                ...
                try{
                    ...
                    $re = $pdo->query("SELECT * FROM `user` WHERE `name`='".$_REQUEST['name']."';");
                    $rows = $re->fetchAll();
                    if($rows == null){
                        ...
                        if($_REQUEST['email'] === $rows[0]['email']){
                            ...
                            //     echo '<p class="narrator" style="font-size: medium; text-align: center; ">这是今日第 '.$row_count.' 次请求, 共5次。</p>';
                                ...
                                $strstr = "echo \"你已经成功更改用户名为：".$_REQUEST['name'].", 邮箱号为：".$_REQUEST['email'].", 你的新密码是：".$genpwd."，如果你的用户之前被锁定，现在已经解锁，可以正常登录。 <weicheng.app用户端邮件已送达>\" | mail -s \"weicheng.app用户端邮件\" ".$_REQUEST['email']."";
                                $result_str = shell_exec($strstr);

                                // power of the second security pass
                                $pdo1 = new pdo('mysql:host=localhost; port=3306; dbname=usertable', 'manager', 'awc020826');
